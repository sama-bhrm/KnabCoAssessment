...
public class ApiLayerExchangeRatesService : AbstractServiceHelper<ExchangeRateRequestDto, List<CurrencyExchangeRate>>,
    ICurrencyExchangeService
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly IOptionsMonitor<ApiLayerExchangeRatesApiSetting> _optionsMonitor;
    ...
    private ApiLayerExchangeRatesApiSetting ApiConfiguration => _optionsMonitor.CurrentValue;

    public ApiLayerExchangeRatesService(IHttpClientFactory httpClientFactory,
        IOptionsMonitor<ApiLayerExchangeRatesApiSetting> optionsMonitor,
    ...
    {
        var majorCurrencyCode = requestDto.CurrencySymbol;
        ...
        if (response.IsSuccessStatusCode)
        {
            ...
            var result = responseDto.Rates.Select(kvp => new CurrencyExchangeRate
                {
    ...
    {
        var client = _httpClientFactory.CreateClient(HttpClientConstants.ApiLayerHttpClientName);

        client.BaseAddress = new Uri(ApiConfiguration.BaseAddress);
        client.DefaultRequestHeaders.Add("apikey", ApiConfiguration.ApiKey);

        var queryString = $"base={requestDto.CurrencySymbol}&symbols={Configuration.MinorCurrencySymbols}";
